  ...
  class ZK
    ...
    def connect
      ...
      unless @omit_ping
        ping_ok = ping
        unless ping_ok
          raise RBZK::ZKNetworkError, "Can't reach device at #{@ip}:#{@port}"
        end
      end

      # Switch to TCP if not forced to use UDP
      unless @force_udp
        begin
          @tcp = TCPSocket.new(@ip, @port)
          @tcp.setsockopt(Socket::SOL_SOCKET, Socket::SO_RCVTIMEO, [ @timeout, 0 ].pack('l_*'))
        rescue => e
          @tcp = nil
          if @verbose
            puts "TCP connection failed: #{e.message}, falling back to UDP"
          end
        end
      end
    ...
    def recv_reply
      ...
      if reply && reply.size >= 8
        ...
        if command == CMD_ACK_OK
          ...
          raise RBZK::ZKErrorResponse, "Error response from device: #{command}"
          ...
          if reply_size > 0
            ...
            raise RBZK::ZKErrorResponse, "Received data packet with no data"
          end
        else
          raise RBZK::ZKErrorResponse, "Invalid response from device: #{command}"
        end
      end
